...
int main() {
  ...
  int i;
  for (i = 0; i < corrupted_bytes.size(); i++) {
    cout << "\r" << i << flush;
    memory[get<1>(corrupted_bytes[i])][get<0>(corrupted_bytes[i])] = true;
    if (bfs(memory, start, end) == -1) {
      break;
    }
  }
  cout << "\r";

  cout << "Coordinates of first byte which prevents exit: (" << get<0>(corrupted_bytes[i]) << "," << get<1>(corrupted_bytes[i]) << ")" << endl;
}
