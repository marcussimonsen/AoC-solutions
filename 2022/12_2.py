...
def BFS(G, V, x, y):
    ...
    while len(Q) != 0:
        ...
        for move in Moves:
            ...
            if V[newY][newX]:
                continue
            # Can't reach check (too high)
            if (G[v.y][v.x] == 'E' and ord(G[newY][newX]) < ord('y')) or G[v.y][v.x] != 'E' and ord(G[newY][newX]) < ord(G[v.y][v.x])-1:
                continue
            # Can't reach end goal check
            if G[newY][newX] == 'S' and ord(G[v.y][v.x]) > ord('b'):
                continue
            V[newY][newX] = True
